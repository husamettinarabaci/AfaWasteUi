<template>
    <b-card no-body>
        <b-card-header>
            <b-card-title>Konteyner Haritası <strong>Toplam: {{ totalCount }}</strong></b-card-title>
        </b-card-header>
...
export default {
    ...
    watch: {
        'show': function(newVal){
            ...
            let tags = this.getTagsFromTruck(newVal);
            this.items = tags;
            this.markers = {};
            if (this.markerGroup){
    ...
    created(){
        let tags = this.getTagsFromTruck(this.show);
        this.items = tags;
    },

    methods: {
        getTagsFromTruck(deviceId){
            let tags = this.$store.getters['panel/getTags'];
            return Object.values(tags).filter(tag => {
                return (tag.DeviceId == deviceId) && (tag.ContainerStatu == Enums.CONTAINER_FULLNESS_STATU_EMPTY);
            })
        },
